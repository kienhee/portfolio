...
@section('content')
    <x-breadcrumb parentName="Quản lý {{ $moduleName }}" parentLink="dashboard.team.index"
        childrenName="Danh sách {{ $moduleName }}" />
    <div class="card">
        <x-alert />
        <x-header-table tableName="Danh sách {{ $moduleName }}" link="dashboard.team.add"
            linkName="Thêm {{ $moduleName }}" />
            ...
            <div class="row ">
                <div class="col-md-6 col-lg-3 mb-2">
                    <div class="input-group input-group-merge">
                        ...
                        <input type="search" class="form-control" placeholder="Tên thành viên" name="keywords"
                ...
                <div class="col-md-6 col-lg-12 mb-2 text-md-end">
                    <a href="{{ route('dashboard.team.index') }}" class="btn btn-outline-secondary">Đặt lại </a>
                        ...
                        @foreach ($teams as $item)
                            <tr>
                                <td class="px-0 text-center">
                                    <a href="{{ route('dashboard.team.edit', $item->id) }}" title="Click xem thêm"
                                        style="color: inherit"><strong>{{ $item->id }}</strong>
                                    </a>
                                </td>
                                <td class="px-0 text-center">
                                    <img src="{{ $item->avatar }}" alt="Ảnh" class=" border rounded w-px-40 h-px-40"
                                        style="object-fit: cover">
                                </td>
                                <td>
                                    <a href="{{ route('dashboard.team.edit', $item->id) }}" style="color: inherit    "
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            <a class="dropdown-item"
                                                href="{{ route('dashboard.team.edit', $item->id) }}"><i
                                            ...
                                            @if ($item->trashed() == 1)
                                                <form class="dropdown-item"
                                                    action="{{ route('dashboard.team.restore', $item->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('delete')
                                            ...
                                            @endif
                                            <form class="dropdown-item"
                                                action="{{ $item->trashed() ? route('dashboard.team.force-delete', $item->id) : route('dashboard.team.soft-delete', $item->id) }}"
                                                method="POST"
